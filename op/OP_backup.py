...
################################################################################
#
# [ Z00 ] Introduction and Credits. 【介绍和鸣谢】
# [ Z01 ] Operation Modifiers. 【操作号注释】
# [ Z02 ] Flow Control. 【流程控制】
# [ Z03 ] Mathematical Operations. 【与数字计算有关的操作号】
# [ Z04 ] Script/Trigger Parameters and Results. 【脚本/触发器的变量和结果】
# [ Z05 ] Keyboard and Mouse Input. 【键盘和鼠标的输入】
# [ Z06 ] World Map. 【大地图】
# [ Z07 ] Game Settings. 【游戏设定】
# [ Z08 ] Factions. 【阵营】
# [ Z09 ] Parties and Party Templates. 【部队和部队模板】
# [ Z10 ] Troops. 【兵种】
# [ Z11 ] Quests. 【任务】
# [ Z12 ] Items. 【物品】
# [ Z13 ] Sounds and Music Tracks. 【声效和歌曲】
# [ Z14 ] Positions. 【位置】
# [ Z15 ] Game Notes. 【游戏记录】
# [ Z16 ] Tableaus and Heraldics. 【画面和纹理】
# [ Z17 ] String Operations. 【字符串相关操作号】
# [ Z18 ] Output And Messages. 【输出和信息】
# [ Z19 ] Game Control: Screens, Menus, Dialogs and Encounters. 【游戏控制：屏幕，菜单，对话，遭遇】
# [ Z20 ] Scenes and Missions. 【场景和剧本】
# [ Z21 ] Scene Props and Prop Instances. 【场景道具和道具实例】
# [ Z22 ] Agents and Teams. 【单位和小组】
# [ Z23 ] Presentations. 【奖励】
# [ Z24 ] Multiplayer And Networking. 【多人游戏和联网】
# [ Z25 ] Remaining Esoteric Stuff. 【剩余的难以领会的东西】
# [ Z26 ] Hardcoded Compiler-Related Code. 【硬编码的链接编辑代码】
#
################################################################################
...
################################################################################
   # Everyone who has ever tried to mod Mount&Blade games knows perfectly well,   【所有玩过骑砍的人都清楚地知道它的模组编程语言的注释是非常缺乏的。战团的模组编程语
   # that the documentation for its Module System is severely lacking. Warband           言在添加了很多新的有用的操作号的同时并没有相对地改进它的注释方法。更糟糕的是，许多
   # Module System, while introducing many new and useful operations, did not          彻底的错误和矛盾出现在header_operations.py file的注释中（无论你是否喜欢它，它都是
   # improve considerably in the way of documentation. What's worse, a number of      所有版本战团的脚本注释的源代码），并且在实际上被应用在了游戏程序中】
   ...
   # Sooner or later someone was bound to dedicate some time and effort to fix          【很快或很久之后必定有人要用很多时间和精力去正确的注释这个文件来修复这个问题。很巧
   # this problem by properly documenting the file. It just so happened that I                的是我是第一个足够疯狂的人来接受这个挑战。】
   # was the first person crazy enough to accept the challenge.

   # I have tried to make this file a self-sufficient source of information on                 【我尝试给这个文件的每一个操作号制作一个自给自足的能让战团脚本程序识别的源代码。理
   # every operation that the Warband scripting engine knows of. Naturally I                 所当然的我失败了----还有很多没有足够信息的代码和效果没有被彻底的测试并确认过的代码
   # failed - there are still many operations for which there is simply not                       。但据我所知，没有别的比这更详细的参考了。我尝试让这个文件对熟练的老手和完全的新
   # enough information, or operations with effects that have not yet been                    手都显得有用，并且在很大的程度上这个文件甚至可以作为通往战团脚本的一个教程，尽管它
   # thoroughly tested and confirmed. But as far as I know, there is currently               仍然不能取代战团mod社区制作的大量教程。】
   ...
   # I really hope you will find it useful as well.                                                            【我确切地希望你能一样觉得它有用。】

   #                                    Alexander Lomski AKA Lav. Jan 18th, 2012.

   # And the credits.【鸣谢翻译略过】
...
neg          = 0x80000000  # (neg|<operation_name>, ...),
                           # Used in combination with conditional operations to invert their results.【用于与条件相关操作号结合来使其结果相反】
this_or_next = 0x40000000  # (this_or_next|<operation_name>, ...),
                           # Used in combination with conditional operations to group them into OR blocks.【用于与条件相关操作号结合来使它们分成非此即彼的两组】
...
call_script             =    1  # (call_script, <script_id>, [<script_param>...]),
                                ...
                                #【调用带或不带变量的脚本。你可以用这个操作号引用最多16个变量。】
try_begin               =    4  # (try_begin),
                                # Opens a conditional block.【开始一个条件块】
else_try                =    5  # (else_try),
                                ...
                                #【在条件块中的条件相关操作号判断失败之前，该代码块将被执行，前提是在该块中没有条件操作，或者它们没有失败。try_for_agents 和 try_for_range也可以                                           造成同样的效果。】
else_try_begin          =    5  # (else_try_begin),
                                # Deprecated form of (else_try).【已弃用】
try_end                 =    3  # (try_end),
                                # Concludes a conditional block or a cycle.【结束一个条件块或循环。】
end_try                 =    3  # (end_try),
                                # Deprecated form of (try_end),【已弃用】
try_for_range           =    6  # (try_for_range, <destination>, <lower_bound>, <upper_bound>),
                                ...
                                #【从<下界>迭代到<<上界> -1>。<destination>是在循环中迭代的变量，它总是在循环结束时递增到下一个值。通过降低上界来打破循环。】
try_for_range_backwards =    7  # (try_for_range_backwards, <destination>, <lower_bound>, <upper_bound>),
                                ...
                                #【与try_for_range 的执行顺序相反。】
try_for_parties         =   11  # (try_for_parties, <destination>),
                                # Runs a cycle, iterating all parties on the map.【运行一个循环，应用于地图上的所有部队】
try_for_agents          =   12  # (try_for_agents, <destination>, [<position_no>], [<radius_fixed_point>]),
                                ...
                                #【运行一个循环，迭代当场的所有被激活的代替物。迭代适用于被激活的或成功或失败的代替物，它只检查位置在周围半径内的那些可选参数。不要使用操作号                                              agent_is_active。它总是返回true。】
